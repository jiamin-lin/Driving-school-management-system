...
<div :style='{"width":"1200px","padding":"20px","margin":"10px auto 20px","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}'  label="考试科目" prop="kaoshikemu">
            <el-select v-model="ruleForm.kaoshikemu" placeholder="请选择考试科目" :disabled=" false  ||ro.kaoshikemu" >
              <el-option
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="安排日期" prop="anpairiqi">
              <el-date-picker
				  :disabled=" false  ||ro.anpairiqi"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`kaoshianpai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('kaoshianpai/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`kaoshianpai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
