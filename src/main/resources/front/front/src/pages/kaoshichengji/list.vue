...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('kaoshichengji','新增')" :style='{"cursor":"pointer","border":"1px solid #769589","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#769589","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"35px","height":"35px"}' type="primary" @click="add('/index/kaoshichengjiAdd')"><i v-if="true" :style='{"color":"#769589","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0","boxShadow":"0px 4px 10px 0px #769589","margin":"10px 1% 16px","flexWrap":"wrap","background":"#fff","display":"flex","width":"31%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"width":"100%","padding":"10px 10px 16px","overflow":"hidden","textAlign":"center","display":"inline-block","height":"auto"}'>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">考试科目:{{item.kaoshikemu}}</div>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">成绩:{{item.chengji}}</div>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">教练姓名:{{item.jiaolianxingming}}</div>
					...
					<div :style='{"padding":"0 5px","display":"inline-block"}'>
					  ...
					  <span :style='{"color":"#666","lineHeight":"1.5","fontSize":"12px"}'>{{item.jiaolianzhanghao}}</span>
					</div>
				</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'考试科目',list:this.kaoshikemuOptions,tableName: 'kaoshikemu',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`kaoshichengji/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/kaoshichengjiDetail', query: params});
      },
