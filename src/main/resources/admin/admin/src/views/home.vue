...
<div class="content" :style='{"minHeight":"100vh","padding":"0px 0px 30px","margin":"4px auto","alignItems":"flex-start","borderRadius":"10px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230831/a5f221858a6744baab19924fb77cee81.png) no-repeat center top / cover","display":"flex","width":"96%","fontSize":"16px","height":"auto"}'>
	...
	<!-- statis -->
	<div :style='{"border":"0px solid #eee","padding":"0px 0","margin":"0 0px 30px 0px","alignItems":"center","color":"#666","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","justifyContent":"center","order":"2"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 20px 10px 0","borderColor":"#dcd6c3","borderRadius":"4px","textAlign":"center","background":"url(http://codegen.caihongy.cn/20230821/2c5d53af6edd48ab9cf77523d4bec3cd.png) no-repeat center bottom / 100% auto,#fff","borderWidth":"1px","display":"flex","width":"calc(20% - 20px)","borderStyle":"solid","justifyContent":"center"}' v-if="isAuth('jiaolianpingjia','首页总数')">
			<div :style='{"alignItems":"center","background":"none","display":"flex","width":"80px","justifyContent":"flex-end","height":"83px","order":"2"}'>
				<span class="icon iconfont icon-tongjitu01" :style='{"color":"inherit","fontSize":"32px"}'></span>
			</div>
			<div :style='{"width":"auto","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{jiaolianpingjiaCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>教练评价总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 1 -->
	<div class="type1" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","height":"455px","order":"10"}'>
		<div id="jiaolianpingjiaChart1" class="echarts1" v-if="isAuth('jiaolianpingjia','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getjiaolianpingjiaCount();
		this.jiaolianpingjiaChat1();
	},
	methods:{
		// 词云
		...
		getjiaolianpingjiaCount() {
			this.$http({
				url: `jiaolianpingjia/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jiaolianpingjiaCount = data.data
				}
			})
		},
		jiaolianpingjiaChat1() {
			this.$nextTick(()=>{

        var jiaolianpingjiaChart1 = echarts.init(document.getElementById("jiaolianpingjiaChart1"),'macarons');
        this.$http({
            url: `jiaolianpingjia/value/jiaolianxingming/pingfen`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiaolianpingjiaChart1.setOption(option);
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, jiaolianpingjiaChart1)
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    jiaolianpingjiaChart1.resize();
                };
            }
        });
      })
