<template>
	<div class="main-content" :style='{"padding":"0px","margin":"0 auto","color":"#333","borderRadius":"0 0 10px 10px","background":"rgba(255,255,255,.0)","width":"96%","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"20px 0 0","overflow":"hidden","color":"#666","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 0 30px","color":"#666","alignItems":"center","display":"flex","-webkitPerspective":"320px","justifyContent":"center","borderRadius":"8px","flexWrap":"wrap","flex":"1","background":"rgba(251,249,243,.9)","width":"auto","perspective":"320px","fontSize":"18px"}'>
					...
					<el-button class="btn18" v-if="isAuth('baomingxinxi','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						批量支付
					</el-button>

					<el-button class="btn18" v-if="isAuth('baomingxinxi','月收入统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('baomingxinxi','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('baomingxinxi','学员加入')" @click="xueyuanxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','已加入')" type="success">
		...
		<xueyuanxinxi-cross-add-or-update v-if="xueyuanxinxiCrossAddOrUpdateFlag" :parent="this" ref="xueyuanxinxiCrossaddOrUpdate"></xueyuanxinxi-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="xuefeiChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import xueyuanxinxiCrossAddOrUpdate from "../xueyuanxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				xueyuanxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			xueyuanxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.xueyuanxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.xueyuanxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.xueyuanxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				if(row.sfsh!='是'){
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var xuefeiChart1 = echarts.init(document.getElementById("xuefeiChart1"),'macarons');
        this.$http({
            url: `baomingxinxi/value/baomingshijian/xuefei/月`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].baomingshijian);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].baomingshijian
                    })
                }
                var option = {};
                option = {
